...
const Transfer = ({ setAction, selectedToken, walletAddress }) => {
    ...
    useEffect(() => {
      ...
      const sendCrypto = async () => {
        console.error('sending crypto')
    
        if (activeTwToken && amount && recipient) {
          setAction('transferring')
          const result = activeTwToken.transfer(
            recipient,
            amount,
          ).then(res => {
            console.error(`Successful transfer: ${result}`)
            setAction('transferred')
          }).catch(err => {
            console.error(`Couldn't send: ${err}`)
          })
        } else {
